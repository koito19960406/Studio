...
Lets recode district names to English
```{r}
unique(Kanagawa_Age_Population$District)
```
```{r}
...
```{r}
p1<-ggplot()
p1+geom_line(data=Districts_far_from_site,aes(x=Year,y=total,group=District_en),colour="grey", alpha=0.3)+
  geom_line(data=Districts_near_site,aes(x=Year,y=total,colour=District_en))+
...
let's 3 years for whole Yokohama
```{r}
Kanagawa_Age_Population %>%
  group_by(District,Year) %>%
  mutate(district_total = sum(Population)) %>% 
